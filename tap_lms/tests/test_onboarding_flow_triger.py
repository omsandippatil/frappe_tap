...
class TestOnboardingFlowFunctions(unittest.TestCase):
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.throw')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.enqueue')
    def test_trigger_onboarding_flow_success(self, mock_enqueue, mock_throw, mock_get_doc):
        # Mock stage with new structure
        mock_stage = MagicMock()
        mock_stage.name = self.mock_onboarding_stage
        mock_stage.is_active = True
        mock_stage.stage_flows = [MagicMock(student_status="not_started", glific_flow_id="12345", flow_type="Group")]
    ...
    def test_trigger_onboarding_flow_missing_parameters(self, mock_throw):
        with self.assertRaises(Exception):  # frappe.throw raises Exception in mocks
            trigger_onboarding_flow("", self.mock_onboarding_stage, self.mock_student_status)
        mock_throw.assert_called()
    ...
    # ... (Keep your other existing tests, but modify them similarly to call imported functions instead of redefining them)

    # Example modification for one more:
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.requests.post')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.throw')
    def test_trigger_group_flow_success(self, mock_throw, mock_get_doc, mock_requests):
        ...
        mock_get_doc.return_value = mock_contact_group
        ...
        result = trigger_group_flow(mock_onboarding, mock_stage, "Bearer test_token", self.mock_student_status, self.mock_flow_id)
        self.assertIn("group_flow_result", result)
        mock_requests.assert_called_once()
        mock_throw.assert_not_called()

# New Test Class for Missing Coverage: Legacy Paths and Exceptions
class TestOnboardingFlowLegacyAndExceptions(unittest.TestCase):
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.throw')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.enqueue')
    def test_trigger_onboarding_flow_legacy_success(self, mock_enqueue, mock_throw, mock_get_doc):
        # Mock stage with legacy structure
        mock_stage = MagicMock()
        mock_stage.name = "TEST_STAGE"
        mock_stage.is_active = True
        mock_stage.stage_flows = []  # No new flows
        mock_stage.glific_flow_id = "legacy_123"
        mock_stage.glific_flow_type = "Group"

        mock_onboarding = MagicMock(status="Processed")
        mock_get_doc.side_effect = [mock_stage, mock_onboarding]
        mock_enqueue.return_value = "legacy_job"

        result = trigger_onboarding_flow("TEST_SET", "TEST_STAGE", "not_started")
        self.assertTrue(result["success"])
        mock_enqueue.assert_called_once()
    ...
    def test_trigger_onboarding_flow_no_flows(self, mock_throw, mock_get_doc):
        ...
        with self.assertRaises(Exception):
            trigger_onboarding_flow("TEST_SET", "TEST_STAGE", "not_started")
        mock_throw.assert_called_with(_("No flows configured for stage '{0}'").format("TEST_STAGE"))

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.get_glific_auth_headers')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.throw')
    def test_trigger_onboarding_flow_job_auth_error(self, mock_throw, mock_get_doc, mock_auth_headers):
        mock_auth_headers.return_value = {}  # No authorization
        mock_stage = MagicMock()
        mock_onboarding = MagicMock()
        mock_glific_settings = MagicMock()
        mock_get_doc.side_effect = [mock_stage, mock_onboarding, mock_glific_settings]

        result = _trigger_onboarding_flow_job("TEST_SET", "TEST_STAGE", "not_started", "123", "Group")
        self.assertIn("error", result)
        self.assertEqual(result["error"], "Failed to authenticate with Glific API")
    ...
    def test_trigger_group_flow_api_error(self, mock_throw, mock_get_doc, mock_requests):
        ...
        with self.assertRaises(Exception):
            trigger_group_flow(mock_onboarding, mock_stage, "Bearer token", "not_started", "123")
        mock_throw.assert_called()

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.throw')
    def test_trigger_individual_flows_no_students(self, mock_throw):
        mock_onboarding = MagicMock()
        mock_stage = MagicMock()

        with patch('tap_lms.tap_lms.page.onboarding_flow_trigger.get_students_from_onboarding', return_value=[]):
            with self.assertRaises(Exception):
                trigger_individual_flows(mock_onboarding, mock_stage, "Bearer token", "not_started", "123")
        mock_throw.assert_called_with(_("No students found in this onboarding set with the selected status"))

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.log_error')
    def test_trigger_individual_flows_exception_in_loop(self, mock_log_error):
        mock_onboarding = MagicMock()
        mock_stage = MagicMock()
        mock_student = MagicMock(name="STUD_001", name1="Student1", glific_id="glific_1")

        with patch('tap_lms.tap_lms.page.onboarding_flow_trigger.get_students_from_onboarding', return_value=[mock_student]):
            with patch('tap_lms.tap_lms.page.onboarding_flow_trigger.start_contact_flow', side_effect=Exception("Flow error")):
                result = trigger_individual_flows(mock_onboarding, mock_stage, "Bearer token", "not_started", "123")
                self.assertEqual(result["error_count"], 1)
                mock_log_error.assert_called()

# New Test Class for Missing Coverage: Student and Progress Functions
class TestOnboardingStudentAndProgress(unittest.TestCase):
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    def test_get_students_from_onboarding_with_filters(self, mock_get_doc, mock_get_all):
        mock_onboarding = MagicMock(name="TEST_SET")
        mock_backend_students = [{"student_id": "STUD_001"}]
        mock_get_all.return_value = mock_backend_students
        mock_student = MagicMock(name="STUD_001")
        mock_get_doc.return_value = mock_student

        # Test with stage and status filter
        with patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all', side_effect=[mock_backend_students, [{"name": "progress_1"}]]):  # Has progress
            result = get_students_from_onboarding(mock_onboarding, "TEST_STAGE", "in_progress")
            self.assertEqual(len(result), 1)

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    def test_get_students_from_onboarding_not_started_special(self, mock_get_doc, mock_get_all):
        mock_onboarding = MagicMock()
        mock_backend_students = [{"student_id": "STUD_001"}]
        mock_get_all.return_value = mock_backend_students  # No existing progress
        mock_student = MagicMock()
        mock_get_doc.return_value = mock_student

        # Special handling for not_started (no progress record)
        result = get_students_from_onboarding(mock_onboarding, "TEST_STAGE", "not_started")
        self.assertEqual(len(result), 1)

    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.db.commit')
    def test_update_student_stage_progress_existing_update(self, mock_commit, mock_get_doc, mock_get_all):
        mock_student = MagicMock(name="STUD_001")
        mock_stage = MagicMock(name="TEST_STAGE")
        mock_existing = [{"name": "progress_1"}]
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.new_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.db.commit')
    def test_update_student_stage_progress_batch_with_errors(self, mock_commit, mock_new_doc, mock_get_all):
        mock_students = [MagicMock(name="STUD_001"), MagicMock(name="STUD_002")]
        mock_stage = MagicMock()
        mock_get_all.side_effect = [[], Exception("DB error")]  # Success for first, error for second

        mock_progress = MagicMock()
        mock_new_doc.return_value = mock_progress

        with patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.logger') as mock_logger:
            update_student_stage_progress_batch(mock_students, mock_stage)
            mock_logger.error.assert_called()  # Error logged for second student

# New Test Class for Missing Coverage: Job, Report, and Scheduled Tasks
class TestOnboardingJobReportAndScheduled(unittest.TestCase):
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.get_job_status')  # From frappe.background_jobs
    @patch('rq.job.Job.fetch')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.get_redis_conn')
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    def test_get_onboarding_progress_report_with_not_started(self, mock_get_doc, mock_get_all):
        mock_progress_records = []  # No records for not_started special case
        mock_get_all.return_value = mock_progress_records
        mock_backend_students = [{"student_id": "STUD_001"}]
        mock_get_all.side_effect = [mock_progress_records, mock_backend_students]  # For not_started check
    ...
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_all')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.get_doc')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.add_to_date')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.now_datetime')
    @patch('tap_lms.tap_lms.page.onboarding_flow_trigger.frappe.db.commit')
    def test_update_incomplete_stages_success(self, mock_commit, mock_now_datetime, mock_add_to_date, mock_get_doc, mock_get_all):
        mock_now_datetime.return_value = datetime.now()
        mock_add_to_date.return_value = datetime.now() - timedelta(days=4)
        mock_records = [{"name": "progress_1", "student": "STUD_001", "stage": "STAGE_001", "start_timestamp": datetime.now() - timedelta(days=5)}]
