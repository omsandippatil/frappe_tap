...
# Mock frappe and related modules before any imports
mock_frappe = Mock()
...
sys.modules['frappe.db'] = Mock()
sys.modules['frappe.logger'] = Mock()

# Mock the glific_integration module to avoid relative import issues
mock_glific_integration = Mock()
mock_glific_integration.get_glific_settings = Mock()
mock_glific_integration.get_glific_auth_headers = Mock()
sys.modules['tap_lms.tap_lms.glific_integration'] = mock_glific_integration
...
try:
    ...
    # If that fails, try to import directly and mock the relative imports
    # First, add the parent directory to path
    parent_dir = os.path.join(os.path.dirname(__file__), '..')
    sys.path.insert(0, parent_dir)
    
    # Mock the relative import
    with patch.dict('sys.modules', {
        'glific_integration': mock_glific_integration,
        '.glific_integration': mock_glific_integration
    }):
    ...
    get_glific_settings = mock_glific_integration.get_glific_settings
    get_glific_auth_headers = mock_glific_integration.get_glific_auth_headers
...
class TestGlificMultiEnrollmentUpdate(unittest.TestCase):
    ...
    @patch('frappe.get_doc')
    @patch('frappe.db.exists')
    def test_update_specific_set_contacts_successful_update(self, mock_exists, mock_get_doc, 
        ...
        mock_exists.return_value = True
        get_glific_settings.return_value = self.mock_settings
        get_glific_auth_headers.return_value = {"Authorization": "Bearer test-token"}
        
        # Mock check_student_multi_enrollment
        with patch('glific_multi_enrollment_update.check_student_multi_enrollment' if 'glific_multi_enrollment_update' in sys.modules 
                  else 'tap_lms.tap_lms.glific_multi_enrollment_update.check_student_multi_enrollment', 
                  return_value="yes"):
    ...
    @patch('frappe.get_doc')
    @patch('frappe.db.exists')
    def test_update_specific_set_contacts_glific_api_error(self, mock_exists, mock_get_doc, 
                                                         mock_get_all, mock_requests_post):
    ...
    def test_run_multi_enrollment_update_for_specific_set_success(self, mock_commit, mock_begin):
        """Test successful whitelist function execution"""
        with patch('glific_multi_enrollment_update.update_specific_set_contacts_with_multi_enrollment' if 'glific_multi_enrollment_update' in sys.modules 
                  else 'tap_lms.tap_lms.glific_multi_enrollment_update.update_specific_set_contacts_with_multi_enrollment') as mock_update:
    ...
    def test_run_multi_enrollment_update_for_specific_set_exception(self, mock_rollback, mock_begin):
    ...
    @patch('time.sleep')
    def test_process_multiple_sets_simple_success(self, mock_sleep):
    ...
    @patch('time.sleep')
    def test_process_multiple_sets_simple_with_error(self, mock_sleep):
    ...
    @patch('time.sleep')
    def test_process_multiple_sets_simple_exception(self, mock_sleep):
