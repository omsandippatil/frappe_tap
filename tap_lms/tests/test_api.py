...
class MockFrappeDocument:
    def __init__(self, doctype, name=None, 
                 ...
                 docstatus=None, idx=None,
                 
                 # API Key fields
                 key=None, enabled=None, api_key_name=None,
                 ...
                 joined_on=None, status=None, enrollment=None,
                 
                 # Teacher fields
                 first_name=None, last_name=None, phone_number=None, email_id=None,
                 subject=None, experience=None, qualification=None, teacher_role=None,
                 department=None, course_level=None,
                 
                 # Batch fields
                 batch_id=None, active=None, regist_end_date=None, start_date=None,
                 end_date=None, capacity=None, enrolled=None,
                 
                 # School fields
                 keyword=None, pin=None, principal_name=None, headmaster_name=None,
                 headmaster_phone=None, model=None, type=None, board=None, country=None,
                 
                 # TAP Language fields
                 language_name=None, glific_language_id=None, language_code=None,
                 is_active=None,
                 
                 # District fields
                 district_name=None, district_code=None,
                 ...
                 # Country fields
                 country_name=None, code=None,
                 
                 # Course Verticals fields
                 name2=None, vertical_name=None, vertical_id=None, description=None,
                 
                 # Course Level fields
                 stage=None, kit_less=None,
                 
                 # Stage Grades fields
                 from_grade=None, to_grade=None, stage_name=None,
                 
                 # Batch onboarding fields
                 batch_skeyword=None, created_by=None,
                 
                 # Batch School Verticals fields
                 course_vertical=None, parent=None,
                 
                 # Gupshup OTP Settings fields
                 api_key=None, source_number=None, app_name=None, api_endpoint=None,
                 template_id=None, is_enabled=None,
                 
                 # Tap Models fields
                 mname=None, model_id=None,
                 
                 # Grade Course Level Mapping fields
                 academic_year=None, assigned_course_level=None, mapping_name=None,
                 
                 # Teacher Batch History fields
                 teacher=None, joined_date=None,
                 
                 # Glific Teacher Group fields
                 glific_group_id=None, label=None,
                 
                 # Enrollment fields
                 course=None, date_joining=None,
                 
                 # OTP Verification fields (commented in original)
                 # phone_number=None, otp=None, expiry=None, verified=None,
                 # context=None, attempts=None, created_at=None
                 ):
        ...
        # Set comprehensive attributes based on doctype
        self._setup_attributes(doctype, 
            # API Key params
            key, enabled, api_key_name,
            # Student params
            name1, student_name, phone, grade, language, school_id, school,
            glific_id, crm_student_id, gender, batch, vertical, student_type,
            district, city, state, pincode, date_of_birth, parent_name,
            parent_phone, email, address, joined_on, status, enrollment,
            # Teacher params
            first_name, last_name, phone_number, email_id, subject,
            experience, qualification, teacher_role, department, course_level,
            # Batch params
            batch_id, active, regist_end_date, start_date, end_date,
            capacity, enrolled,
            # School params
            keyword, pin, principal_name, headmaster_name, headmaster_phone,
            model, type, board, country,
            # TAP Language params
            language_name, glific_language_id, language_code, is_active,
            # District params
            ...
            # Country params
            country_name, code,
            # Course Verticals params
            name2, vertical_name, vertical_id, description,
            # Course Level params
            stage, kit_less,
            # Stage Grades params
            from_grade, to_grade, stage_name,
            # Batch onboarding params
            batch_skeyword, created_by,
            # Batch School Verticals params
            course_vertical, parent,
            # Gupshup OTP Settings params
            api_key, source_number, app_name, api_endpoint, template_id, is_enabled,
            # Tap Models params
            mname, model_id,
            # Grade Course Level Mapping params
            academic_year, assigned_course_level, mapping_name,
            # Teacher Batch History params
            teacher, joined_date,
            # Glific Teacher Group params
            glific_group_id, label,
            # Enrollment params
            course, date_joining
        )
    
    def _setup_attributes(self, doctype, 
                         # API Key params
                         key, enabled, api_key_name,
                         # Student params
                         name1, student_name, phone, grade, language, school_id, school,
        ...
        if doctype == "API Key":
            ...
            self.language_name = language_name or 'English'
            self.glific_language_id = glific_language_id or '1'
            self.language_code = language_code or 'en'
            self.is_active = is_active if is_active is not None else 1
            
        elif doctype == "District":
            self.district_name = district_name or 'Test District'
            self.state = state or 'Test State'
            self.district_code = district_code or 'TD001'
            
        elif doctype == "City":
            self.city_name = city_name or 'Test City'
            self.district = district or 'Test District'
            self.state = state or 'Test State'
            self.city_code = city_code or 'TC001'
            
        elif doctype == "State":
            self.state_name = state_name or 'Test State'
            self.country = country or 'India'
            self.state_code = state_code or 'TS'
            
        elif doctype == "Country":
            self.country_name = country_name or 'India'
            self.code = code or 'IN'
            
        elif doctype == "Course Verticals":
            self.name2 = name2 or 'Math'
            self.vertical_name = vertical_name or 'Mathematics'
            self.vertical_id = vertical_id or 'VERT_001'
            self.description = description or 'Mathematics subject'
            self.is_active = is_active if is_active is not None else 1
            
        elif doctype == "Course Level":
            self.name1 = name1 or 'Beginner Math'
            self.vertical = vertical or 'VERTICAL_001'
            self.stage = stage or 'STAGE_001'
            self.kit_less = kit_less if kit_less is not None else 1
            
        elif doctype == "Stage Grades":
            self.from_grade = from_grade or '1'
            self.to_grade = to_grade or '5'
            self.stage_name = stage_name or 'Primary'
            
        elif doctype == "Batch onboarding":
            self.batch_skeyword = batch_skeyword or 'test_batch'
            self.school = school or 'SCHOOL_001'
            self.batch = batch or 'BATCH_001'
            self.kit_less = kit_less if kit_less is not None else 1
            self.model = model or 'MODEL_001'
            self.is_active = is_active if is_active is not None else 1
            self.created_by = created_by or 'Administrator'
            self.from_grade = from_grade or '1'
            self.to_grade = to_grade or '10'
            
        elif doctype == "Batch School Verticals":
            self.course_vertical = course_vertical or 'VERTICAL_001'
            self.parent = parent or 'BATCH_ONBOARDING_001'
            
        elif doctype == "Gupshup OTP Settings":
            self.api_key = api_key or 'test_gupshup_key'
            self.source_number = source_number or '918454812392'
            self.app_name = app_name or 'test_app'
            self.api_endpoint = api_endpoint or 'https://api.gupshup.io/sm/api/v1/msg'
            self.template_id = template_id or 'template_123'
            self.is_enabled = is_enabled if is_enabled is not None else 1
            
        elif doctype == "Tap Models":
            self.mname = mname or 'Test Model'
            self.model_id = model_id or 'MODEL_001'
            self.description = description or 'Test model description'
            
        elif doctype == "Grade Course Level Mapping":
            self.academic_year = academic_year or '2025-26'
            self.course_vertical = course_vertical or 'VERTICAL_001'
            self.grade = grade or '5'
            self.student_type = student_type or 'New'
            self.assigned_course_level = assigned_course_level or 'COURSE_001'
            self.mapping_name = mapping_name or 'Test Mapping'
            self.is_active = is_active if is_active is not None else 1
            
        elif doctype == "Teacher Batch History":
            self.teacher = teacher or 'TEACHER_001'
            self.batch = batch or 'BATCH_001'
            self.batch_id = batch_id or 'BATCH_2025_001'
            self.status = status or 'Active'
            ...
            self.batch = batch or 'BATCH_001'
            self.glific_group_id = glific_group_id or 'GROUP_001'
            self.label = label or 'teacher_batch_001'
            
        elif doctype == "Enrollment":
            self.batch = batch or 'BATCH_001'
            self.course = course or 'COURSE_001'
            self.grade = grade or '5'
            self.date_joining = date_joining or datetime.now().date()
            self.school = school or 'SCHOOL_001'
            self.parent = parent or 'STUDENT_001'
...
class MockFrappe:
    ...
    def _configure_get_doc(self):
        def get_doc_side_effect(doctype, filters=None, name=None, creation=None, modified=None, 
                               owner=None, modified_by=None, docstatus=None, idx=None,
                               key=None, enabled=None, api_key_name=None, name1=None, 
                               student_name=None, phone=None, grade=None, language=None,
                               school_id=None, school=None, glific_id=None, crm_student_id=None,
                               gender=None, batch=None, vertical=None, student_type=None,
                               district=None, city=None, state=None, pincode=None, 
                               date_of_birth=None, parent_name=None, parent_phone=None, 
                               email=None, address=None, joined_on=None, status=None, 
                               enrollment=None, first_name=None, last_name=None, 
                               phone_number=None, email_id=None, subject=None, 
                               experience=None, qualification=None, teacher_role=None, 
                               department=None, course_level=None, batch_id=None, 
                               active=None, regist_end_date=None, start_date=None, 
                               end_date=None, capacity=None, enrolled=None, keyword=None, 
                               pin=None, principal_name=None, headmaster_name=None, 
                               headmaster_phone=None, model=None, type=None, board=None, 
                               country=None, language_name=None, glific_language_id=None, 
                               language_code=None, is_active=None, district_name=None, 
                               district_code=None, city_name=None, city_code=None, 
                               state_name=None, state_code=None, country_name=None, 
                               code=None, name2=None, vertical_name=None, vertical_id=None, 
                               description=None, stage=None, kit_less=None, from_grade=None, 
                               to_grade=None, stage_name=None, batch_skeyword=None, 
                               created_by=None, course_vertical=None, parent=None, 
                               api_key=None, source_number=None, app_name=None, 
                               api_endpoint=None, template_id=None, is_enabled=None, 
                               mname=None, model_id=None, academic_year=None, 
                               assigned_course_level=None, mapping_name=None, teacher=None, 
                               joined_date=None, glific_group_id=None, label=None, 
                               course=None, date_joining=None):
            
            if doctype == "API Key":
                if isinstance(filters, dict):
                    key_val = filters.get('key')
                elif isinstance(filters, str):
                    key_val = filters
                else:
                    key_val = key or 'unknown_key'
                
                if key_val in ['valid_key', 'test_key']:
                    return MockFrappeDocument(doctype, key=key_val, enabled=1)
                elif key_val == 'disabled_key':
                    return MockFrappeDocument(doctype, key=key_val, enabled=0)
                else:
                    raise self.DoesNotExistError("API Key not found")
            
            elif doctype == "OTP Verification":
                if isinstance(filters, dict):
                    phone_val = filters.get('phone_number')
                    if phone_val == '9876543210':
                        return MockFrappeDocument(doctype, phone_number='9876543210')
                    elif phone_val == 'expired_phone':
                        return MockFrappeDocument(doctype, phone_number='expired_phone')
                    elif phone_val == 'verified_phone':
                        return MockFrappeDocument(doctype, phone_number='verified_phone')
                    else:
                        raise self.DoesNotExistError("OTP Verification not found")
                else:
                    raise self.DoesNotExistError("OTP Verification not found")
                ...
                if isinstance(filters, dict):
                    ...
                    name1_val = filters.get('name1')
                    if keyword_val == 'test_school' or name1_val == 'Test School':
                        return MockFrappeDocument(doctype, keyword='test_school', name1='Test School')
                elif isinstance(filters, str):
                    return MockFrappeDocument(doctype, name=filters)
                else:
                ...
                return MockFrappeDocument(doctype, name=name, batch_id=batch_id, active=active,
                                        regist_end_date=regist_end_date, school=school,
                                        start_date=start_date, end_date=end_date,
                                        capacity=capacity, enrolled=enrolled)
                
            elif doctype == "Tap Models":
                return MockFrappeDocument(doctype, name=name, mname=mname, model_id=model_id,
                                        description=description)
                
            elif doctype == "City":
                return MockFrappeDocument(doctype, name=name, city_name=city_name,
                                        district=district, state=state, city_code=city_code)
                
            elif doctype == "District":
                return MockFrappeDocument(doctype, name=name, district_name=district_name,
                                        state=state, district_code=district_code)
                
            elif doctype == "State":
                return MockFrappeDocument(doctype, name=name, state_name=state_name,
                                        country=country, state_code=state_code)
            
            return MockFrappeDocument(doctype, name=name)
    ...
    def _configure_db_operations(self):
        ...
        def db_sql_side_effect(query, params=None):
            if "Stage Grades" in query:
                return [{'name': 'STAGE_001'}]
            elif "Teacher Batch History" in query:
                return [{'batch': 'BATCH_001', 'batch_name': 'Test Batch', 
...
def safe_call_function(func, *args, **remaining_kwargs):
    """Safely call a function and return result or exception info"""
    try:
