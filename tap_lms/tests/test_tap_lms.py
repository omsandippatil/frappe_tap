...
class TestTapLmsConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock the entire frappe module before any imports
        self.frappe_mock = MagicMock()
        self.frappe_mock._ = MagicMock(return_value="Mocked translation")
        ...
        modules_to_clear = [key for key in sys.modules.keys() if key.startswith('tap_lms')]
    ...
    def tearDown(self):
        ...
        # Remove frappe from sys.modules
        if 'frappe' in sys.modules:
            del sys.modules['frappe']
        
        # Clear tap_lms modules
        modules_to_clear = [key for key in sys.modules.keys() if key.startswith('tap_lms')]
        for module in modules_to_clear:
            if module in sys.modules:
                del sys.modules[module]
   
    def test_import_frappe_underscore(self):
        """Test that frappe._ is imported successfully"""
        # Import the module to trigger the import statement
        from tap_lms.config import tap_lms
    ...
    def test_get_data_function_exists_and_callable(self):
        """Test that get_data function exists and is callable"""
        # Import the module
        from tap_lms.config import tap_lms
    ...
    def test_frappe_underscore_called_with_correct_arguments(self):
        """Test that frappe._ is called with the correct arguments"""
        # Import and call the function
        from tap_lms.config import tap_lms
        tap_lms.get_data()
        ...
        # Verify frappe._ was called with expected arguments
        expected_calls = ["School", "Manage School"]
        actual_calls = [call[0][0] for call in self.frappe_mock._.call_args_list]
